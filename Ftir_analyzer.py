...
if uploaded_file is not None:
    # Check if the file is a document PDF instead of an image graph
    if uploaded_file.name.lower().endswith('.pdf'):
        st.warning("⚠️ **PDF Document Detected:** The analytical matrix requires a direct visual graph image to trace sub-pixel lines. Please upload a screenshot or image export (.png, .jpg, or .webp) of your spectrum to run the digitizer.")
    else:
        ...
        if len(scan_x) > 0:
            ...
            def generate_quick_ftir_pdf(data_lines):
                ...
                for title, points, _ in data_lines:
                    ...
                    pdf.set_font("Helvetica", "", 10)
                    # multi_cell automatically calculates width constraints and wraps the lines
