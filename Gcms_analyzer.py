...
if uploaded_file is not None:
    # Handle PDF document edge case safely
    if uploaded_file.name.lower().endswith('.pdf'):
        st.warning("⚠️ **PDF Document Detected:** The analytical matrix requires a direct visual graph image to trace chromatographic tracks. Please upload a screenshot or image export (.png, .jpg, or .webp) of your curve to run the digitizer.")
    else:
        ...
        if len(scan_x) > 0:
            ...
            if has_peaks:
                ...
                with c2:
                    ...
                    st.download_button(label="📘 Download Detailed Report (PDF)", data=bytes(pdf_detailed), file_name="gcms_comprehensive_report.pdf", mime="application/pdf")
else:
