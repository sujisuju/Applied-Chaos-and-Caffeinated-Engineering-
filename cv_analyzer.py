...
# Flexible multi-extension format support matrix
uploaded_file = st.file_uploader(
    "📂 Upload Voltammetry Curve (PNG, JPG, JPEG, WEBP, PDF)", 
    type=["png", "jpg", "jpeg", "webp", "pdf", "PNG", "JPG", "JPEG", "WEBP", "PDF"]
)

if uploaded_file is not None:
    if uploaded_file.name.lower().endswith('.pdf'):
        st.warning("⚠️ **PDF Document Detected:** The analytical matrix requires a direct visual graph image to trace electrochemical tracks. Please upload a screenshot or image export (.png, .jpg, or .webp) of your voltammetry curve to run the digitizer.")
    else:
        ...
        if len(scan_x) > 0:
            ...
            for batch_name, peaks in cv_batches.items():
                if len(peaks) > 0:
                    ...
                    peak_list_text = "  |  ".join([f"{v} V (Current: {curr})" for v, curr in unique_peaks])
