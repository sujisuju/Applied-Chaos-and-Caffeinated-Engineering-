...
with tab_home:
    ...
    with col1:
        st.markdown('<div class="feature-card"><div class="card-icon">🔮</div><div class="card-title">FTIR Optical Suite</div><div class="card-desc">Leverages computer vision matrix thresholding to isolate infrared baseline curves, reconstructing physical absorbance data points from flat screenshots.</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="feature-card"><div class="card-icon">⚗️</div><div class="card-title">Voltammetry Sweep Engine</div><div class="card-desc">Maps dynamic faradaic redox curves to calculate potential intervals, isolating anodic and cathodic sweep interactions directly from instrument captures.</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div class="feature-card"><div class="card-icon">🧪</div><div class="card-title">GCMS Resolution Hub</div><div class="card-desc">Groups chromatogram retention intervals into descriptive batch zones, evaluating solvent purity, degradation traces, and macro-structural signatures.</div></div>', unsafe_allow_html=True)

# ==========================================
# 📊 EXTERNAL ROUTING GATEWAYS
# ==========================================
with tab_ftir:
    try:
        exec(open("Ftir_analyzer.py").read())
    except Exception as e:
        st.error(f"FTIR Module Loading Interrupted: {e}")

with tab_cv:
    try:
        exec(open("cv_analyzer.py").read())
    except Exception as e:
        st.error(f"Voltammetry Module Loading Interrupted: {e}")
